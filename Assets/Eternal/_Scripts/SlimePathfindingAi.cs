...
public class SlimePathfindingAi : EnemyBaseAI {
	...
	void FixedUpdate()
	{
		...
		else if(hit == true)
		{
			...
			if(relativePoint.x > 0.0&&facingRight||relativePoint.x < 0.0&&!facingRight)
				transform.position = new Vector3(transform.position.x - movespeed*amountPushBack, transform.position.y);
	...
	private void ApplyDamage(object[] damages)
	{
		...
		if ((float)damages [5] > 0f&&!isPoisonous) {
			poisoned = true;
