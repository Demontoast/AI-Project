...
public class TabooSwordPathfindingAi : EnemyBaseAI {
	...
	void FixedUpdate()
	{
		...
		if (dead == false&&agro&&!hit) {
			...
			} else if (Distance > atkZone && Distance < atkZone + 3f&& a <= 0&&!block&&!dodge&&d<=0) {
				Face ();
				//m = 6;
				//i = 3;
			...
			else if (dodge&&Distance > atkZone) {
				if (facingRight == true)
					transform.position = new Vector3 (transform.position.x + 0.15f, transform.position.y);
				else
					transform.position = new Vector3 (transform.position.x - 0.15f, transform.position.y);
			...
			else if (Distance > atkZone && Distance < walkZone&& m <= 0&&!block) {
				WalkTowards (Distance);
			}

			else if (Distance > walkZone && i <= 0&& m <= 0&&!block) {
				StartPathFinding ();
				i = 3;
			}
			else if (path.size () > 0&& m <= 0&&!block) {
				Move ();
			} else{
				anim.SetBool ("Move", false);
				if (Distance > walkZone&&m<=0&&!block) {
					StartPathFinding ();
					i = 3;
				}
			}
	...
	void Move()
	{
		...
		} else if (TargetDist >= 1 && (currentNode.transform.position.x > groundCheck.transform.position.x && currentNode.transform.position.x - groundCheck.transform.position.x > 1f || currentNode.transform.position.x < groundCheck.transform.position.x && groundCheck.transform.position.x - currentNode.transform.position.x > 1f)) {
	...
	{
		if (Distance >= 1f && (Player.transform.position.x > this.transform.position.x && Player.transform.position.x - this.transform.position.x > 1f || Player.transform.position.x < this.transform.position.x && this.transform.position.x - Player.transform.position.x > 1f)) {
			anim.SetBool ("Move", true);
			if (facingRight == true)
				transform.position = new Vector3 (transform.position.x + movespeed, transform.position.y);
			else
