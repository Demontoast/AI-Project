...
public class TribesmanPathfindingAI : EnemyBaseAI {
	...
	void Start()
	{
		...
		spear = Resources.Load ("TribesmanSpear")as GameObject;
		isHpVis(false);
	...
	void FixedUpdate()
	{
		...
		if (dead == false&&agro&&!hit) {
			...
			if (Distance <= rangeAtkZone && Distance >= atkZone && a <= 0) {
				Face ();
			...
			else if (Distance > atkZone && Distance < walkZone&& m <= 0) {
	...
	void Shoot()
	{
		Transform firePoint = this.transform.FindChild ("Throw");
		GameObject bul = Instantiate(spear, firePoint.position,spear.transform.rotation)as GameObject;
		if(facingRight == true)
		{
